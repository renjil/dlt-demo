...
CREATE STREAMING LIVE TABLE raw_ txs
...
AS SELECT txs.*, ref.id as accounting_treatment FROM stream(LIVE.raw_txs) txs
...
AS SELECT l.* FROM LIVE.reference_loan_stats l
  INNER JOIN LIVE.ref_accounting_treatment ref USING (id)
